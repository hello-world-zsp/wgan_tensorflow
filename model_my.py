...
class MYDCGAN (object):
    ...
    def generator(self, gdata):
        ...
        with tf.variable_scope("generator") as scope:
            self.z_, self.h0_w, self.h0_b = linear(gdata, self.gf_dim * 4 * s8 * s8,
                                                   'g_h0_lin', with_w=True)
            self.h0 = tf.reshape(self.z_, [-1, s8, s8, self.gf_dim * 4])      # batch_size*5*5*512
            h0 = tf.nn.relu(self.g_bn0(self.h0))

            self.h1, self.h1_w, self.h1_b = deconv2d(h0,[self.batch_size, s4, s4, self.gf_dim * 2],
                                                     name='g_h1',with_w=True)
            h1 = tf.nn.relu(self.g_bn1(self.h1))

            h2, self.h2_w, self.h2_b = deconv2d(h1,[self.batch_size, s2, s2, self.gf_dim * 1],
                                                name='g_h2', with_w=True)
            h2 = tf.nn.relu(self.g_bn2(h2))

            h3, self.h3_w, self.h3_b = deconv2d(h2,[self.batch_size, s, s, self.c_dim],
